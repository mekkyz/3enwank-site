...
export function ThemeSwitch({ label, dark, light }: { label: string; dark: string; light: string }) {
  ...
  return (
    <div role="group" aria-label={label} className="inline-flex rounded-lg border border-line bg-panel p-0.5 text-xs font-bold">
      {(["dark", "light"] as const).map((t) => (
        <button key={t} type="button" aria-pressed={theme === t} onClick={() => apply(t)} className={`inline-flex min-h-8 items-center gap-1.5 rounded-md px-3 transition ${theme === t ? "bg-brand-soft text-brand-strong" : "text-muted hover:text-ink"}`}>
          {t === "dark" ? (
