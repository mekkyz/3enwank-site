 ...
 * is the alternative for someone who would rather write it down, not the main event. Under it sits
 * the one route that must not be buried: a customer whose site is down should not be filling in a
 * sales form and waiting for working hours.
 */
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1.05fr] lg:items-start">
        <div className="order-1 space-y-6">
          <Panel tone="brand">
            ...
            {whatsapp ? (
              <>
                <p className="mt-4">
                  ...
                  <a
                    href={`tel:+${whatsapp}`}
                    className="tabular text-3xl font-extrabold text-ink hover:text-brand-strong sm:text-4xl"
                  >
            ...
            {/*
             * Both languages shown: a reader should see their own on the page before deciding which to
             * write in. `dir` on the paragraph would right-align the Arabic against the far side of
             * the panel, where it reads as a stray caption rather than as the second half of a pair,
             * so <bdi> isolates the run instead and both lines start at the same edge.
             */}
          ...
          <Panel tone="warn">
            ...
            <ul className="mt-3 flex flex-wrap gap-x-5 gap-y-1 text-sm font-bold">
              {whatsapp ? (
                <li>
                  <a
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-3">
        <Panel>
          ...
          <a
            ...
            className="mt-3 block break-all text-lg font-extrabold text-brand-strong hover:underline"
            dir="ltr"
          >
            {contactEmail}
          </a>
        ...
        <Panel>
          ...
          <p className="mt-3">
            <a
              href={`${storeUrl}/login`}
              rel="noopener"
              className="text-sm font-bold text-brand-strong hover:text-brand"
            >
              {c.existingCta}
            </a>
          </p>
